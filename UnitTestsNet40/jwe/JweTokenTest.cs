...
namespace UnitTests
{
	...
	{ 		
		...
		public void ParseJsonEncodingSingleRecipient()
		{
			...
			Assert.That(test.Encoding, Is.EqualTo(SerializationMode.Json));
			Assert.That(test.ProtectedHeaderBytes, Is.EqualTo(new byte[] { 123, 34, 116, 121, 112, 34, 58, 34, 74, 87, 69, 34, 125 }));
			Assert.That(test.Ciphertext, Is.EqualTo(new byte[] { 207, 222, 111, 60, 159, 224, 95, 23, 163, 164, 91, 39, 163, 209, 1, 9 }));
			Assert.That(test.Iv, Is.EqualTo(new byte[] { 140, 103, 108, 108, 216, 229, 251, 251, 135, 79, 133, 124, 232, 199, 69, 4 }));
			Assert.That(test.AuthTag, Is.EqualTo(new byte[] { 113, 178, 137, 98, 158, 25, 69, 101, 143, 89, 81, 195, 200, 189, 175, 185, 72, 192, 103, 122, 0, 29, 116, 245, 35, 190, 109, 214, 63, 107, 8, 162 }));
			Assert.Null(test.Aad);
			Assert.That(test.UnprotectedHeader.Count, Is.EqualTo(1));
			Assert.That(test.UnprotectedHeader["enc"], Is.EqualTo("A256CBC-HS512"));


			Assert.That(test.Recipients.Count, Is.EqualTo(1));

			Assert.That(test.Recipients[0].EncryptedCek, Is.EqualTo(new byte[] { 42, 154, 250, 20, 117, 149, 136, 147, 103, 24, 43, 131, 16, 73, 118, 237, 219, 2, 139, 37, 135, 70, 56, 152, 184, 29, 157, 58, 171, 158, 211, 170, 42, 37, 145, 149, 98, 11, 189, 202, 39, 244, 47, 163, 138, 119, 216, 22, 190, 91, 107, 24, 151, 104, 253, 188, 154, 124, 97, 121, 151, 2, 47, 114, 94, 153, 24, 35, 95, 68, 93, 236 }));
			Assert.That(test.Recipients[0].Header.Count, Is.EqualTo(1));
			Assert.That(test.Recipients[0].Header["alg"], Is.EqualTo("PBES2-HS256+A128KW"));
		}
		...
		public void ParseJsonEncoding()
		{
			...
			Assert.That(test.Recipients[0].EncryptedCek, Is.EqualTo(new byte[] { 42, 154, 250, 20, 117, 149, 136, 147, 103, 24, 43, 131, 16, 73, 118, 237, 219, 2, 139, 37, 135, 70, 56, 152, 184, 29, 157, 58, 171, 158, 211, 170, 42, 37, 145, 149, 98, 11, 189, 202, 39, 244, 47, 163, 138, 119, 216, 22, 190, 91, 107, 24, 151, 104, 253, 188, 154, 124, 97, 121, 151, 2, 47, 114, 94, 153, 24, 35, 95, 68, 93, 236 }));
			Assert.That(test.Recipients[0].Header.Count, Is.EqualTo(3));
			Assert.That(test.Recipients[0].Header["alg"], Is.EqualTo("PBES2-HS256+A128KW"));
			Assert.That(test.Recipients[0].Header["p2c"], Is.EqualTo(8192));
			Assert.That(test.Recipients[0].Header["p2s"], Is.EqualTo("C5Hn0y-ho1mwygXPVfDynQ"));

			Assert.That(test.Recipients[1].EncryptedCek, Is.EqualTo(new byte[] { 86, 236, 207, 162, 189, 78, 17, 233, 207, 63, 236, 52, 170, 15, 255, 184, 100, 180, 195, 136, 122, 98, 222, 202, 217, 145, 237, 206, 58, 163, 210, 105, 192, 206, 19, 115, 76, 123, 166, 96, 88, 218, 138, 249, 52, 117, 76, 36, 212, 125, 163, 198, 39, 132, 59, 209, 88, 116, 46, 60, 4, 227, 12, 140, 199, 50, 150, 117, 70, 78, 0, 21 }));
			Assert.That(test.Recipients[1].Header.Count, Is.EqualTo(2));
			Assert.That(test.Recipients[1].Header["alg"], Is.EqualTo("ECDH-ES+A128KW"));
			var epk = (IDictionary<string, object>)test.Recipients[1].Header["epk"];
			Assert.That(epk.Count, Is.EqualTo(4));
			Assert.That(epk["crv"], Is.EqualTo("P-256"));
			Assert.That(epk["kty"], Is.EqualTo("EC"));
			Assert.That(epk["x"], Is.EqualTo("LqM-HYhs3GcIPKRdiR2R7CuPx-aPVwBohgzP9l2WdfA"));
			Assert.That(epk["y"], Is.EqualTo("0hP45SduS8HPQaZ8RAyikZTuvYCjKaknhcCSVK_tIIY"));
		}
		...
		public void SerializeJsonEncodingSingleRecipient()
		{
			...
			// then
			Assert.That(@"{""ciphertext"":""z95vPJ_gXxejpFsno9EBCQ"",""protected"":""eyJ0eXAiOiJKV0UifQ"",""iv"":""jGdsbNjl-_uHT4V86MdFBA"",""tag"":""cbKJYp4ZRWWPWVHDyL2vuUjAZ3oAHXT1I75t1j9rCKI"",""header"":{""alg"":""PBES2-HS256+A128KW""},""encrypted_key"":""Kpr6FHWViJNnGCuDEEl27dsCiyWHRjiYuB2dOque06oqJZGVYgu9yif0L6OKd9gWvltrGJdo_byafGF5lwIvcl6ZGCNfRF3s""}", Is.EqualTo(test));
		}
		...
		public void SerializeCompact()
		{
			...
			//then 
			Assert.That("eyJhbGciOiJSU0ExXzUiLCJlbmMiOiJBMTI4R0NNIn0.FojyyzygtFOyNBjzqTRfr9HVHPrvtqbVUt9sXSuU59ZhLlzk7FrirryFnFGtj8YC9lx-IX156Ro9rBaJTCU_dfERd05DhPMffT40rdcDiLxfCLOY0E2PfsMyGQPhI6YtNBtf_sQjXWEBC59zH_VoswFAUstkvXY9eVVecoM-W9HFlIxwUXMVpEPtS96xZX5LMksDgJ9sYDTNa6EQOA0hfzw07fD_FFJShcueqJuoJjILYbad-AHbpnLTV4oTbFTYjskRxpEYQr9plFZsT4_xKiCU89slT9EFhmuaiUI_-NGdX-kNDyQZj2Vtid4LSOVv5kGxyygThuQb6wjr1AGe1g.O92pf8iqwlBIQmXA.YdGjkN7lzeKYIv743XlPRYTd3x4VA0xwa5WVoGf1hiHlhQuXGEg4Jv3elk4JoFJzgVuMMQMex8fpFFL3t5I4H9bH18pbrEo7wLXvGOsP971cuOOaXPxhX6qClkwx5qkWhcTbO_2AuJxzIaU9qBwtwWaxJm9axofAPYgYbdaMZkU4F5sFdaFY8IOe94wUA1Ocn_gxC_DYp9IEAyZut0j5RImmthPgiRO_0pK9OvusE_Xg3iGfdxu70x0KpoItuNwlEf0LUA.uP5jOGMxtDUiT6E3ubucBw", Is.EqualTo(test));
		}
	}
}
